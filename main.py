...
NOTIFIED_FILE = "last_notified.json"
...
def convert_timestamp(ts):
    ...
    try:
        ...
        dt = datetime.strptime(ts, "%Y-%m-%dT%H:%M:%S.%fZ")  # With microseconds

    except ValueError:

        try:

            dt = datetime.strptime(ts, "%Y-%m-%dT%H:%M:%SZ")  # Without microseconds

        except ValueError:

            return "N/A"  # If timestamp is invalid



    return f"<t:{int(dt.timestamp())}:F>"

async def send_notification(session, name, price, description, wide_image_url, tall_image_url, activation_date, expiration_date):
    ...
    embed = {
        ...
        # Use the wide image for the main image

        "image": {"url": wide_image_url} if wide_image_url.startswith(("http://", "https://")) else None,



        # Use the tall image for the top-right thumbnail
    ...
    async with session.get(FORTNITE_PACKS_URL, headers=headers) as response:
        ...
        for pack in items:
            ...
            if old_data.get(pack_name) != {
                ...
                await send_notification(session, pack_name, pack_price, pack_description, wide_image_url, tall_image_url, activation_date, expiration_date)
